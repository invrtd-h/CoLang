...
def test10(): Unit = {
  춘잣! (
    "2차원 벡터" 하는 플레이보이 예전에 ("뭉", "탱") 하더놈 같은데 이제 (
      "내적" 중에는!! 아무리 "우변"이라도 (
        ((나 의 "뭉" 감동님 사랑해) 조이고 ("우변"의 "뭉" 감동님 사랑해)) 배
          ((나 의 "탱" 감동님 사랑해) 조이고 ("우변"의 "탱" 감동님 사랑해))
      ) 할 수가 없단다!!
    ) 하는갑지 돈좀버냐?? 개노잼 노라라??,
    Construct(StringID("2차원 벡터"), Vector(1, 2))의 "내적" 감동님 사랑해 야
      Construct(StringID("2차원 벡터"), Vector(3, 4)) 먹어라?? 리액션 "%s\n"
  )
}
